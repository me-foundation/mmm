import {
  ...
  import { BubblegumTreeRef, CNFT, CnftDepositSellArgs } from '../../sdk/src';
  ...
  export async function setupTreeAndListing(
    umi: Umi,
    ...
    const sellArgs: CnftDepositSellArgs = {
      nft: {
        tree: await getBubblegumTreeRef(),
        nft: getCnftRef(proof),
      },
      seller: getPubKey(seller.publicKey),
      nftDelegate: getPubKey(seller.publicKey),
      price: 100,
    };
  
    //   await runSell(umi, m3Client, seller, sellArgs);
  
    // Verify that M3 program owns the cNFT.
    const { currentProof: escrowedProof } = await verifyOwnership(
      umi,
      merkleTree,
      seller.publicKey, // !!!! this need to be changed to the pool?
      leafIndex,
      updatedMetadata,
      [],
    );
