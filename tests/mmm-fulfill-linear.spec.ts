...
describe('mmm-fulfill-linear', () => {
  ...
  it('Sellside only', async () => {
    ...
    assert.equal(poolAccountInfo.spotPrice.toNumber(), 1.1 * LAMPORTS_PER_SOL);
    ...
    assert.equal(poolAccountInfo.spotPrice.toNumber(), 2.3 * LAMPORTS_PER_SOL);
    assert.equal(poolAccountInfo.lpFeeEarned.toNumber(), 0);
    ...
    poolAccountInfo = await program.account.pool.fetch(poolData.poolKey);
    assert.equal(poolAccountInfo.spotPrice.toNumber(), 2.4 * LAMPORTS_PER_SOL);
    assert.equal(poolAccountInfo.lpFeeEarned.toNumber(), 0);
  ...
  it('Buyside only', async () => {
...

    {
      ...
      assert.equal(referralBalance, initReferralBalance + expectedReferralFees);
      assert.equal(poolAtaBalance, tokenAccountRent);
      assert.equal(poolEscrowBalance, 6.8 * LAMPORTS_PER_SOL - totalMakerFees);
      assert.equal(afterWalletBalance, initWalletBalance);
    ...
    poolAccountInfo = await program.account.pool.fetch(poolData.poolKey);
    assert.equal(poolAccountInfo.spotPrice.toNumber(), 0.4 * LAMPORTS_PER_SOL);
  ...
  it('Two sides', async () => {
...

    {
      ...
      const expectedReferralFees = expectedSellTakerFees + expectedMakerFees;
